...
import json
from .repos_fichas import FichasRepo

repo = FichasRepo()

@require_http_methods(["GET"])
def fs_fichas_list(request):
    try:
...
def fs_fichas_create(request):
    try:
        data = json.loads(request.body or "{}")
        created = repo.create_ficha(data)
...
def fs_fichas_detail(request, doc_id: str):
    ...
    if request.method == "PUT":
        try:
            data = json.loads(request.body or "{}")
            obj = repo.update_ficha(doc_id, data)
