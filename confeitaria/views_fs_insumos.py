...
import json
from .repos_insumos import InsumosRepo

repo = InsumosRepo()

@require_http_methods(["GET"])
def fs_insumos_list(request):
    try:
...
def fs_insumos_create(request):
    try:
        data = json.loads(request.body or "{}")
        created = repo.create_insumo(data)
...
def fs_insumos_detail(request, doc_id: str):
    ...
    if request.method == "PUT":
        try:
            data = json.loads(request.body or "{}")
            obj = repo.update_insumo(doc_id, data)
