...
import json
from .repos_saidas import SaidasRepo

repo = SaidasRepo()

@require_http_methods(["GET"])
def fs_saidas_list(request):
    try:
...
def fs_saidas_create(request):
    try:
        data = json.loads(request.body or "{}")
        created = repo.create_saida(data)
...
def fs_saidas_detail(request, doc_id: str):
    ...
    if request.method == "PUT":
        try:
            data = json.loads(request.body or "{}")
            obj = repo.update_saida(doc_id, data)
